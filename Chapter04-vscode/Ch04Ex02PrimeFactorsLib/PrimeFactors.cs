namespace Ch04Ex02PrimeFactorsLib
{
    ...
    {
        public static int[] Primeinputs = new[]
    {
        ...
        public static string GetPrimeFactors(int input)
        {
            if ((input < 1) || (input > 1000))
            {
                return $"{nameof(input)} must be between 1 and 1000.";
            }

            string factors = string.Empty;

            foreach (int divisor in Primeinputs)
            {
                int remainder;
                do
                {
